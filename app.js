...
const FABRICATION_API = window.FABRICATION_API_URL || "https://kiss-nearly-broken-perth.trycloudflare.com";
...
async function loadAssemblyMarks() {
  ...
  let savedRows = [];
  if (FABRICATION_API) { const params = new URLSearchParams({ projectId: projectId() || "", fileId: $("ifcSelect").value || "", assemblyName: assembly.assemblyName }); try { const response = await fetch(`${FABRICATION_API.replace(/\/$/, "")}/fabrication-details/list?${params}`); if (response.ok) savedRows = await response.json(); } catch (error) { console.warn("Could not load saved fabrication details", error); } }
  const rowData = marks.map(markEntry => { const mark = typeof markEntry === "string" ? markEntry : markEntry.mark; const metrics = typeof markEntry === "string" ? markMetrics.get(`${assembly.assemblyName}|${mark}`) || {} : markEntry; const assignment = savedAssignments.find(item => (item.AssemblyMark || item.assemblyMark || item.mark) === mark && (!(item.AssemblyName || item.assemblyName || item.assembly_name) || (item.AssemblyName || item.assemblyName || item.assembly_name) === assembly.assemblyName)); const fabrication = savedRows.find(item => item.mark === mark); return { ...assignment, ...fabrication, mark, modelQuantity: metrics.quantity ?? assignment?.quantity, eachWeight: metrics.eachWeight ?? assignment?.eachWeight ?? metrics.weight ?? assignment?.weight, totalWeight: metrics.totalWeight ?? ((metrics.quantity || 0) * (metrics.eachWeight || metrics.weight || 0)) }; });
