...
async function loadAssemblyMarks() {
  ...
  let savedRows = [];
  if (FABRICATION_API) { const listParams = new URLSearchParams({ projectId: projectId() || "", fileId: $("ifcSelect").value || "", assemblyName: assembly.assemblyName }); try { const response = await fetch(`${FABRICATION_API.replace(/\/$/, "")}/fabrication-details/list?${listParams}`); if (response.ok) savedRows = await response.json(); } catch (error) { console.warn("Could not list saved fabrication details", error); } }
  const rowData = await Promise.all(marks.map(async markEntry => { const mark = typeof markEntry === "string" ? markEntry : markEntry.mark; const metrics = typeof markEntry === "string" ? markMetrics.get(`${assembly.assemblyName}|${mark}`) || {} : markEntry; const assignment = savedAssignments.find(item => (item.AssemblyMark || item.assemblyMark || item.mark) === mark && (!(item.AssemblyName || item.assemblyName || item.assembly_name) || (item.AssemblyName || item.assemblyName || item.assembly_name) === assembly.assemblyName)); let fabrication = savedRows.find(item => String(item.mark || "") === String(mark)) || {}; if (!Object.keys(fabrication).length && FABRICATION_API) { const params = new URLSearchParams({ projectId: projectId() || "", fileId: $("ifcSelect").value || "", assemblyName: assembly.assemblyName, mark }); try { const response = await fetch(`${FABRICATION_API.replace(/\/$/, "")}/fabrication-details?${params}`); if (response.ok) fabrication = await response.json(); } catch (error) { console.warn("Could not load saved fabrication details", error); } } const saved = { fabricatorName: fabrication.fabricator_name ?? fabrication.FabricatorName, completionDate: fabrication.completion_date ?? fabrication.CompletionDate, quantity: fabrication.quantity ?? fabrication.Quantity, weight: fabrication.weight ?? fabrication.Weight }; return { ...assignment, ...fabrication, mark, saved, modelQuantity: metrics.quantity ?? assignment?.quantity, eachWeight: metrics.eachWeight ?? assignment?.eachWeight ?? metrics.weight ?? assignment?.weight, totalWeight: metrics.totalWeight ?? ((metrics.quantity || 0) * (metrics.eachWeight || metrics.weight || 0)) }; }));
  const existingMarks = new Set(rowData.map(item => String(item.mark)));
  for (const fabrication of savedRows) if (fabrication.mark && !existingMarks.has(String(fabrication.mark))) rowData.push({ ...fabrication, mark: fabrication.mark, saved: { fabricatorName: fabrication.fabricator_name, completionDate: fabrication.completion_date, quantity: fabrication.quantity, weight: fabrication.weight } });
  $("markRows").innerHTML = rowData.length ? rowData.map(item => { const qrData = qrUrl(item.mark, item); const eachWeight = Number(item.eachWeight); const totalWeight = Number(item.totalWeight); const savedWeight = Number(item.saved.weight); const completed = item.saved.completionDate ? String(item.saved.completionDate).slice(0, 10) : ""; return `<tr><td>${item.mark}</td><td>${item.modelQuantity ?? ""}</td><td>${Number.isFinite(eachWeight) && eachWeight ? `${eachWeight.toFixed(3)} t` : ""}</td><td>${Number.isFinite(totalWeight) && totalWeight ? `${totalWeight.toFixed(3)} t` : ""}</td><td>${item.saved.fabricatorName || ""}</td><td>${completed}</td><td>${item.saved.quantity ?? ""}</td><td>${Number.isFinite(savedWeight) ? `${savedWeight.toFixed(3)} t` : ""}</td><td><div class="qr-code" id="qr-${encodeURIComponent(item.mark)}"></div></td><td><button type="button" class="print-qr" data-qr="${encodeURIComponent(qrData)}">Print</button></td></tr>`; }).join("") : "<tr><td colspan=\"10\">No marks found</td></tr>";
