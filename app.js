...
async function loadAssemblyMarks() {
  ...
  const rowData = await Promise.all(marks.map(async markEntry => { const mark = typeof markEntry === "string" ? markEntry : markEntry.mark; const metrics = typeof markEntry === "string" ? markMetrics.get(`${assembly.assemblyName}|${mark}`) || {} : markEntry; const assignment = savedAssignments.find(item => (item.AssemblyMark || item.assemblyMark || item.mark) === mark && (!(item.AssemblyName || item.assemblyName || item.assembly_name) || (item.AssemblyName || item.assemblyName || item.assembly_name) === assembly.assemblyName)); let fabrication = {}; if (FABRICATION_API) { const params = new URLSearchParams({ projectId: projectId() || "", fileId: $("ifcSelect").value || "", assemblyName: assembly.assemblyName, mark }); try { const response = await fetch(`${FABRICATION_API.replace(/\/$/, "")}/fabrication-details?${params}`); if (response.ok) fabrication = await response.json(); } catch (error) { console.warn("Could not load saved fabrication details", error); } } return { ...assignment, ...fabrication, mark, modelQuantity: metrics.quantity ?? assignment?.quantity, modelWeight: metrics.weight ?? assignment?.weight }; }));
  $("markRows").innerHTML = rowData.length ? rowData.map(item => { const qrData = qrUrl(item.mark, item); const modelWeight = Number(item.modelWeight); const savedWeight = Number(item.weight); const completed = item.completion_date ? String(item.completion_date).slice(0, 10) : ""; return `<tr><td>${item.mark}</td><td>${item.modelQuantity ?? ""}</td><td>${Number.isFinite(modelWeight) && modelWeight ? `${modelWeight.toFixed(3)} t` : ""}</td><td>${item.fabricator_name || ""}</td><td>${completed}</td><td>${item.quantity ?? ""}</td><td>${Number.isFinite(savedWeight) && savedWeight ? `${savedWeight.toFixed(3)} t` : ""}</td><td><div class="qr-code" id="qr-${encodeURIComponent(item.mark)}"></div></td><td><button type="button" class="print-qr" data-qr="${encodeURIComponent(qrData)}">Print</button></td></tr>`; }).join("") : "<tr><td colspan=\"9\">No marks found</td></tr>";
